  ...
  return (
    <Box>
      <Box sx={{ display: "flex", justifyContent: "center", mb: 3 }}>
        <img
          src={process.env.PUBLIC_URL + "/assets/profile_pic.jpeg"}
          alt="Profilbild"
          style={{
            width: 180,
            height: 180,
      ...
      <Stack spacing={1} mb={3}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography variant="body2">{contact.location.label}</Typography>
        </Box>
        {contact.linkedin && (
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <LinkedIn fontSize="small" />
            <Typography variant="body2">{contact.linkedin.label}</Typography>
          </Box>
        )}
        {contact.github && (
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <GitHub fontSize="small" />
            <Typography variant="body2">{contact.github.label}</Typography>
          </Box>
        )}
      </Stack>
      <Divider sx={{ my: 2 }} />
      ...
      <Box sx={{ mb: 3 }}>
        {data.languages.map((language, idx) => (
          <Box
            key={idx}
            sx={{ display: "grid", gridTemplateColumns: "80px 1fr", mb: 1 }}
          >
